...
print(f"--- EmbyPulse V12 (Receipt Edition) Starting ---")
print(f"DB Path: {DB_PATH}")
if not os.path.exists(DB_PATH):
    print(f"⚠️ 严重警告: 找不到数据库文件! 请检查 Docker 映射路径是否为 {DB_PATH}")
...
async def api_get_users():
    try:
        ...
        if not results: 
            print("⚠️ [API] 数据库中没有播放记录 (PlaybackActivity 表为空)")
        ...
        print(f"✅ [API] 找到 {len(data)} 个用户")
        return {"status": "success", "data": data}
    except Exception as e: return {"status": "error", "message": str(e)}

# 保留原有 Dashboard 接口，确保首页正常
...
async def api_poster_data(user_id: Optional[str] = None, period: str = 'all'):
    ...
    try:
        ...
        print(f"   🔎 [Poster] 查找到 {len(rows) if rows else 0} 条原始记录")
        ...
        return {
            ...
            "data": {
                ...
                "top_list": top_list,
                "tags": ["观影新人"] if total_plays < 10 else ["资深影迷"]
            }
        }
    except Exception as e:
        ...
        return {
            "status": "error",
            "message": str(e),
            "data": {"plays": 0, "hours": 0, "server_plays": 0, "top_list": [], "tags": []}
        }
...
async def api_user_details(user_id: Optional[str] = None):
    return {"status": "success", "data": {"hourly": {}, "devices": [], "logs": []}}
